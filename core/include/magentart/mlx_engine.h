...
public:
  ...
  /// @name SpectroStream RVQ token manipulation
  /// Atomic UI setters. Values are normalized `0..1` and operate on raw
  /// SpectroStream RVQ token frames before they are forced back through the
  /// exported sampler/decoder path.
  /// @{
  void set_rvq_force(float v);
  float get_rvq_force() const;
  void set_rvq_breathe(float v);
  float get_rvq_breathe() const;
  void set_rvq_memory(float v);
  float get_rvq_memory() const;
  void set_rvq_coarse(float v);
  float get_rvq_coarse() const;
  void set_rvq_fine(float v);
  float get_rvq_fine() const;
  void set_rvq_sweep(float v);
  float get_rvq_sweep() const;
  void set_rvq_hold(float v);
  float get_rvq_hold() const;
  void set_rvq_invert(float v);
  float get_rvq_invert() const;
  void set_rvq_jitter(float v);
  float get_rvq_jitter() const;
  void set_rvq_stride(float v);
  float get_rvq_stride() const;
  /// @}
  /// @}
  ...
  /// Copy the cached MusicCoCa conditioning embedding for prompt slot `index`,
  /// regardless of whether it came from text, audio, or an explicit vector.
