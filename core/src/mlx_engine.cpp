...
int32_t wrap_rvq_code(int64_t v) {
  const int64_t vocab = static_cast<int64_t>(kVocabSize);
  v %= vocab;
  if (v < 0) {
    v += vocab;
  }
...
struct MLXEngine::Impl {
  ...
  bool build_forced_rvq_tokens(std::array<int32_t, kNumRVQLevels> &out_raw);
  void remember_rvq_tokens(const int32_t *raw_tokens);
  uint32_t next_rvq_u32();
  float next_rvq_float();
  ...
  bool get_prompt_embedding(int index, float *out) const;
  ...
  std::atomic<float> rvq_force_{0.0f};
  std::atomic<float> rvq_breathe_{0.0f};
  std::atomic<float> rvq_memory_{0.0f};
  std::atomic<float> rvq_coarse_{0.0f};
  std::atomic<float> rvq_fine_{0.0f};
  std::atomic<float> rvq_sweep_{0.0f};
  std::atomic<float> rvq_hold_{0.0f};
  std::atomic<float> rvq_invert_{0.0f};
  std::atomic<float> rvq_jitter_{0.0f};
  std::atomic<float> rvq_stride_{0.0f};
  std::array<int32_t, kNumRVQLevels> last_rvq_raw_{};
  std::array<int32_t, kNumRVQLevels> held_rvq_raw_{};
  static constexpr std::size_t kRvqMemoryFrames = 512;
  std::array<std::array<int32_t, kNumRVQLevels>, kRvqMemoryFrames>
      rvq_token_ring_{};
  std::size_t rvq_token_head_ = 0;
  std::size_t rvq_token_count_ = 0;
  bool has_last_rvq_raw_ = false;
  bool has_held_rvq_raw_ = false;
  uint64_t rvq_frame_counter_ = 0;
  uint32_t rvq_rng_state_ = 0x9e3779b9u;
...
void MLXEngine::Impl::reset_state() {
  ...
  has_last_rvq_raw_ = false;
  has_held_rvq_raw_ = false;
  rvq_token_head_ = 0;
  rvq_token_count_ = 0;
  rvq_frame_counter_ = 0;
  rvq_rng_state_ = 0x9e3779b9u ^ static_cast<uint32_t>(
                                      seed_rotation_.load(std::memory_order_relaxed));
...
uint32_t MLXEngine::Impl::next_rvq_u32() {
  uint32_t x = rvq_rng_state_;
  if (x == 0) {
    x = 0x9e3779b9u;
  }
  x ^= x << 13;
...
float MLXEngine::Impl::next_rvq_float() {
  return static_cast<float>(next_rvq_u32() >> 8) * (1.0f / 16777216.0f);
}

void MLXEngine::Impl::remember_rvq_tokens(const int32_t *raw_tokens) {
  if (!raw_tokens) {
    return;
  }
  for (int k = 0; k < static_cast<int>(kNumRVQLevels); ++k) {
    last_rvq_raw_[k] = wrap_rvq_code(raw_tokens[k]);
  }
  has_last_rvq_raw_ = true;
  if (!has_held_rvq_raw_) {
    held_rvq_raw_ = last_rvq_raw_;
    has_held_rvq_raw_ = true;
  }
  rvq_token_ring_[rvq_token_head_] = last_rvq_raw_;
  rvq_token_head_ = (rvq_token_head_ + 1) % kRvqMemoryFrames;
  rvq_token_count_ = std::min(rvq_token_count_ + 1, kRvqMemoryFrames);
}

bool MLXEngine::Impl::build_forced_rvq_tokens(
    std::array<int32_t, kNumRVQLevels> &out_raw) {
  if (!has_last_rvq_raw_) {
    return false;
  }

  const float force = clamp01(rvq_force_.load(std::memory_order_relaxed));
  const float breathe = clamp01(rvq_breathe_.load(std::memory_order_relaxed));
  const float memory = clamp01(rvq_memory_.load(std::memory_order_relaxed));
  const float coarse = clamp01(rvq_coarse_.load(std::memory_order_relaxed));
  const float fine = clamp01(rvq_fine_.load(std::memory_order_relaxed));
  const float sweep = clamp01(rvq_sweep_.load(std::memory_order_relaxed));
  const float hold = clamp01(rvq_hold_.load(std::memory_order_relaxed));
  const float invert = clamp01(rvq_invert_.load(std::memory_order_relaxed));
  const float jitter = clamp01(rvq_jitter_.load(std::memory_order_relaxed));
  const float stride = clamp01(rvq_stride_.load(std::memory_order_relaxed));
  if (force < 0.001f) {
    return false;
  }
  const float activity =
      std::max({force, memory, coarse, fine, sweep, hold, invert, jitter, stride});
  if (activity < 0.001f) {
    return false;
  }

  const uint64_t frame = rvq_frame_counter_++;

  if (breathe > 0.001f) {
    const float duty = 1.0f - breathe * 0.86f;
    const int cycle_frames =
        4 + static_cast<int>(std::round(breathe * 46.0f));
    const int on_frames =
        std::max(1, static_cast<int>(std::round(cycle_frames * duty)));
    const bool rhythmic_on =
        static_cast<int>(frame % static_cast<uint64_t>(cycle_frames)) < on_frames;
    const bool stray_hit = next_rvq_float() < duty * 0.10f * activity;
    if (!rhythmic_on && !stray_hit) {
      return false;
    }
  }
  ...
  out_raw = last_rvq_raw_;
  const int stride_step = 1 + static_cast<int>(std::round(stride * 127.0f));

  if (memory > 0.001f && rvq_token_count_ > 2) {
    const std::size_t available =
        std::min(rvq_token_count_, kRvqMemoryFrames);
    const std::size_t max_lookback =
        std::max<std::size_t>(
            2, std::min<std::size_t>(
                   available - 1,
                   2 + static_cast<std::size_t>(
                           std::round(memory * memory *
                                      static_cast<float>(available - 2)))));
    const uint64_t scan =
        frame * static_cast<uint64_t>(stride_step) +
        static_cast<uint64_t>(std::round(sweep * 127.0f)) +
        static_cast<uint64_t>(std::round(hold * 31.0f));
    const std::size_t lookback =
        1 + static_cast<std::size_t>(scan % max_lookback);
    const std::size_t index =
        (rvq_token_head_ + kRvqMemoryFrames - lookback) % kRvqMemoryFrames;
    out_raw = rvq_token_ring_[index];
  }

  for (int k = 0; k < static_cast<int>(kNumRVQLevels); ++k) {
    const bool is_coarse = k < 4;
    const float band = is_coarse ? coarse : fine;
    float target = std::max(band, force * (is_coarse ? 0.55f : 0.35f));
    if (band < 0.001f) {
      target = std::max(target, activity * 0.45f);
    }
    target = clamp01(target);

    int64_t code = out_raw[k];
    if (hold > 0.001f) {
      if (next_rvq_float() < hold * target) {
        code = held_rvq_raw_[k];
      } else if (next_rvq_float() < (1.0f - hold) * 0.08f) {
        held_rvq_raw_[k] = out_raw[k];
      }
    }

    if (band > 0.001f && next_rvq_float() < band * target * 0.75f) {
      code = static_cast<int32_t>(next_rvq_u32() % kVocabSize);
    }

    if (sweep > 0.001f && next_rvq_float() < sweep * target) {
      const int64_t phase = static_cast<int64_t>(frame * stride_step);
      code = phase + k * 83 + ((phase >> ((k % 5) + 1)) & 0x3ff);
    }

    if (stride > 0.001f) {
      const int64_t offset =
          static_cast<int64_t>(std::round(stride * stride * 384.0f)) *
          static_cast<int64_t>((k + 1) * ((frame % 7) + 1));
      code += offset;
    }

    if (invert > 0.001f && next_rvq_float() < invert * target) {
      code = static_cast<int64_t>(kVocabSize - 1) - wrap_rvq_code(code);
    }

    if (jitter > 0.001f && next_rvq_float() < std::max(jitter, target)) {
      const int span =
          1 + static_cast<int>(std::round(jitter * jitter * 511.0f));
      const int delta = static_cast<int>(next_rvq_u32() % (span * 2 + 1)) - span;
      code += delta;
    }

    if (force > 0.001f && next_rvq_float() < force * 0.08f) {
      code = static_cast<int32_t>(next_rvq_u32() % kVocabSize);
    }

    out_raw[k] = wrap_rvq_code(code);
  }
...
bool MLXEngine::Impl::generate_frame(float *audio_L, float *audio_R,
  ...
  std::array<int32_t, kNumRVQLevels> forced_raw;
  std::vector<int32_t> forced_unique;
  std::vector<int32_t> empty_forced_tokens;
  if (build_forced_rvq_tokens(forced_raw)) {
    forced_unique.resize(kNumRVQLevels);
    for (int k = 0; k < static_cast<int>(kNumRVQLevels); ++k) {
      forced_unique[k] = forced_raw[k] +
                         k * static_cast<int32_t>(kVocabSize) +
                         (kNumReservedTokens - 1);
    }
    persistent_args_[8] =
        mx::array(forced_unique.data(),
                  {1, 1, static_cast<int>(kNumRVQLevels)}, mx::int32);
  } else {
    persistent_args_[8] =
        mx::array(empty_forced_tokens.data(),
                  {1, 0, static_cast<int>(kNumRVQLevels)}, mx::int32);
  }
  ...
  // Extract the just-sampled or just-forced raw RVQ tokens. The
  ...
  // convert back to per-codebook raw codes here so the token bender and
  // optional caller get tidy `[0..kVocabSize)` values per codebook.
  if (previous_frame_state_idx_ >= 0 &&
    ...
    // For both rank-3 [1, 1, Q] and rank-4 [1, CFG, 1, Q] the model
    // samples once per CFG group and broadcasts; the first Q ints
    ...
    std::array<int32_t, kNumRVQLevels> raw_tokens;
    for (int k = 0; k < static_cast<int>(kNumRVQLevels); ++k) {
      raw_tokens[k] =
          wrap_rvq_code(p[k] - k * static_cast<int32_t>(kVocabSize) -
                        (kNumReservedTokens - 1));
    }
    remember_rvq_tokens(raw_tokens.data());
    if (tokens_out) {
      for (int k = 0; k < static_cast<int>(kNumRVQLevels); ++k) {
        tokens_out[k] = raw_tokens[k];
      }
    }
  }
...
bool MLXEngine::Impl::get_audio_embedding(int index, float *out) const {
  ...
  if (!slot_is_audio_[index])
    return false;
  if (out) {
    memcpy(out, cached_embeddings_[index],
           kMusicCoCaEmbeddingDim * sizeof(float));
  }
  return true;
}

bool MLXEngine::Impl::get_prompt_embedding(int index, float *out) const {
  if (index < 0 || index >= (int)kMaxPrompts)
    return false;
  std::lock_guard<std::mutex> lock(const_cast<std::mutex &>(musiccoca_mutex_));
  if (!embedding_valid_[index])
    return false;
  if (out) {
    memcpy(out, cached_embeddings_[index],
           kMusicCoCaEmbeddingDim * sizeof(float));
...
void MLXEngine::set_rvq_force(float v) {
  impl_->rvq_force_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_force() const {
  return impl_->rvq_force_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_breathe(float v) {
  impl_->rvq_breathe_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_breathe() const {
  return impl_->rvq_breathe_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_memory(float v) {
  impl_->rvq_memory_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_memory() const {
  return impl_->rvq_memory_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_coarse(float v) {
  impl_->rvq_coarse_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_coarse() const {
  return impl_->rvq_coarse_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_fine(float v) {
  impl_->rvq_fine_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_fine() const {
  return impl_->rvq_fine_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_sweep(float v) {
  impl_->rvq_sweep_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_sweep() const {
  return impl_->rvq_sweep_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_hold(float v) {
  impl_->rvq_hold_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_hold() const {
  return impl_->rvq_hold_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_invert(float v) {
  impl_->rvq_invert_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_invert() const {
  return impl_->rvq_invert_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_jitter(float v) {
  impl_->rvq_jitter_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_jitter() const {
  return impl_->rvq_jitter_.load(std::memory_order_relaxed);
}
void MLXEngine::set_rvq_stride(float v) {
  impl_->rvq_stride_.store(clamp01(v), std::memory_order_relaxed);
}
float MLXEngine::get_rvq_stride() const {
  return impl_->rvq_stride_.load(std::memory_order_relaxed);
}
...
bool MLXEngine::get_prompt_embedding(int i, float *o) const {
  return impl_->get_prompt_embedding(i, o);
}
