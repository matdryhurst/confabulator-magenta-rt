...
+ (float)defaultDrumlessForPrefix:(NSString*)prefixString {
    return [prefixString isEqualToString:@"Confabulator"] ? 1.0f : 0.0f;
}
...
+ (BOOL)shouldPersistParam:(int)address {
    return (address >= 0 && address <= 9 && address != 2) || address == 39 || address == 47 || address == 48;
}
...
+ (void)applyParamToEngine:(RealtimeRunner*)engine
    ...
    else if (address == 47) engine->set_seed_rotation((int)value);
...
+ (float)readParamFromEngine:(RealtimeRunner*)engine
    ...
    else if (address == 47) return (float)engine->get_seed_rotation();
...
+ (void)restoreSavedParams:(RealtimeRunner*)engine
    ...
    float bufferSize = [prefixString isEqualToString:@"Confabulator"] ? kConfabulatorDefaultBufferSize : kMagentaDefaultBufferSize;
    for (int i = 0; i <= 100; i++) {
        if (![self shouldPersistParam:i]) continue;
        NSString* key = [self paramKeyForAddress:i];
        if (!key) continue;
        NSString* defaultsKey = [NSString stringWithFormat:@"%@_Param_%@", prefixString, key];
        if (i == 39 && [prefixString isEqualToString:@"Confabulator"]) {
            NSString* migrationKey = @"Confabulator_DrumlessDefaultV2";
            if (![defaults objectForKey:migrationKey]) {
                [self applyParamToEngine:engine address:i value:1.0f prefixString:prefixString];
        ...
        if ([defaults objectForKey:defaultsKey]) {
            ...
            else if (i == 8) value = bufferSize;
            else if (i == 39) value = [self defaultDrumlessForPrefix:prefixString];
            else if (i == 47) value = 0.0f;
...
+ (void)resetDefaultsOnEngine:(RealtimeRunner*)engine
    ...
    [self applyParamToEngine:engine address:39 value:[self defaultDrumlessForPrefix:prefixString] prefixString:prefixString]; // drumless
    [self applyParamToEngine:engine address:47 value:0.0f                       prefixString:prefixString]; // seed rotation
