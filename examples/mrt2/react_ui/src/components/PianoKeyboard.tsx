...
interface PianoKeyboardProps {
  ...
  /** Font size for key labels in px (default 11) */
  octaveLabelFontSize?: number;
  /** MIDI note number for the QWERTY keyboard base (determines which keys get labels) */
  keyboardBaseNote?: number;
}
...
export function PianoKeyboard({
  ...
  for (let n = startNote; n <= endNote; n++) {
    if (!BLACK_KEYS.has(n % 12)) {
      const offset = n - keyboardBaseNote;
